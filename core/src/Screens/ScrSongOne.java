...
public class ScrSongOne extends InputAdapter implements Screen {
    ...
    private Rectangle recRED, recBLUE, recGREEN, recPURPLE, recHUD;
    ...
    public void show() {
        ...
        ArsCols[2] = "GREEN";
        ArsCols[3] = "PURPLE";
        recRED = new Rectangle(0, 0, w / 2, h / 2); //rectangles representing -
        recBLUE = new Rectangle(w / 2, 0, w / 2, h / 2); // - the corners
        recGREEN = new Rectangle(0, h / 2, w / 2, h / 2);
        recPURPLE = new Rectangle(w / 2, h / 2, w / 2, h / 2);
        recHUD = new Rectangle(0, h - (h / 4.2f), w, (h / 4.2f) - 25);
        AlRandRect.add(recGREEN);//TL //Populating the arraylist with rectangles
        AlRandRect.add(recPURPLE);//TR
        AlRandRect.add(recRED);//BL
        AlRandRect.add(recBLUE);//BR
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (i < 2) {
                ArSprGrey[i].setPosition(AlRandRect.get(i + 2).x, AlRandRect.get(i + 2).y);
            } else {
                ArSprGrey[i].setPosition(AlRandRect.get(i - 2).x, AlRandRect.get(i - 2).y);
            }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            ArSprColours[i].setPosition(AlRandRect.get(i).x, AlRandRect.get(i).y);
    ...
    public void render(float delta) {
        ...
        if (!isDone && !isPause) { //Playing Game
            ...
            if (nCountSwitch == nRand) {//change the colour locations
                ...
                ArSprColours[0].setPosition(recGREEN.x, recGREEN.y);
                ArSprColours[1].setPosition(recPURPLE.x, recPURPLE.y);
                ArSprColours[2].setPosition(recRED.x, recRED.y);
                ArSprColours[3].setPosition(recBLUE.x, recBLUE.y);
            }
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if (!isDone) {//decide if the click was correct
            if (button == Buttons.LEFT && !circ.contains(screenX, screenY)) {
                if (recRED.contains(screenX, screenY) && ArcRandCol[nNext] == Color.RED) {
                    isCorrect = true;
                } else if (recBLUE.contains(screenX, screenY) && ArcRandCol[nNext] == Color.BLUE) {
                    isCorrect = true;
                } else if (recGREEN.contains(screenX, screenY) && ArcRandCol[nNext] == Color.GREEN) {
                    isCorrect = true;
                } else if (recPURPLE.contains(screenX, screenY) && ArcRandCol[nNext] == Color.PURPLE) {
                    isCorrect = true;
                }
            }
